...
import { fields, invalidData, invalidSchemaErrorData, schema, schemaError, validData, validSchemaErrorData, } from './__fixtures__/data';
...
describe('valibotResolver', () => {
    it('should return parsed values from valibotResolver with `mode: sync` when validation pass', async () => {
        vi.mock('valibot', async () => {
            const a = await vi.importActual('valibot');
            return {
                __esModule: true,
                ...a,
            };
    ...
    it('should return all the errors from valibotResolver when validation fails with `validateAllFieldCriteria` set to true and `mode: sync`', async () => {
        const result = await valibotResolver(schema, undefined, { mode: 'sync' })(invalidData, undefined, {
            fields,
            criteriaMode: 'all',
            shouldUseNativeValidation,
        });
    ...
    it('should correctly infer the output type from a valibot schema using a transform', () => {
        const resolver = valibotResolver(v.object({
            id: v.pipe(v.number(), v.transform((val) => String(val))),
        }));
        expectTypeOf(resolver).toEqualTypeOf();
    });
