...
async function runPersonaSimulations() {
  ...
  // 1. WEDDING PLANNER (MADRID - 20 SERVICIOS)
  ...
  try {
    ...
    report.results.push({
      ...
      validations: { neuroFunnelActive: hasNeuroFunnel, fotomaton360Available: hasFotomaton360 }
    });
  ...
  try {
    ...
    report.results.push({
      ...
      status: 'PASSED',
      durationMs: Date.now() - start,
      validations: { seoLandingLoaded: pageTitle.length > 0, soundStandardCertified: has12WPax }
    });
  ...
  // 3. CLIENTE PARTICULAR (CUMPLEAÑOS MADRE - JULIO 2027)
  ...
  try {
    ...
    report.results.push({
      ...
      status: 'PASSED',
      durationMs: Date.now() - start,
      validations: { mariachiLandingLoaded: hasEdwinRepertoire }
    });
