...
VAULT_INPUT_DIR = r"H:\00_PRODUCTORA_EAR\EAR_ABSORBED_VAULT\05_CODIGO_Y_SISTEMA"
SYSTEM_VAULT_DIR = r"H:\EAR_OS_V2\EAR_OS_V2\vault\proveedores_html_indexados"

CATEGORY_MAP = {
    'musica': 'Música',
    'animacion': 'Animación & Espectáculos',
    'fotografos': 'Fotografía & Vídeo',
    'video': 'Fotografía & Vídeo',
    'catering': 'Catering & Banquetes',
    'banquetes': 'Fincas & Espacios',
    'fincas': 'Fincas & Espacios',
    'hoteles': 'Fincas & Espacios',
    'restaurantes': 'Fincas & Espacios',
    'decoracion-para-bodas': 'Decoración & Flores',
    'flores': 'Decoración & Flores',
    'coches-de-boda': 'Transporte & Coches',
    'autobuses': 'Transporte & Coches',
    'trajes-madrina': 'Moda & Belleza',
    'talleres-de-novia': 'Moda & Belleza',
    'belleza-novias': 'Moda & Belleza',
    'joyeria': 'Moda & Belleza',
    'detalles-de-boda': 'Servicios & Regalos',
    'invitaciones-de-boda': 'Servicios & Regalos',
    'organizacion-bodas': 'Wedding Planners',
    'viaje-de-novios': 'Servicios & Regalos',
    'carpas': 'Infraestructura & Sonido',
    'sonido': 'Infraestructura & Sonido'
}

SUBFOLDER_MAP = {
    'Música': 'Musica',
    'Fotografía & Vídeo': 'Fotografia_Video',
    'Catering & Banquetes': 'Catering_Banquetes',
    'Fincas & Espacios': 'Fincas_Espacios',
    'Decoración & Flores': 'Decoracion_Flores',
    'Animación & Espectáculos': 'Animacion_Espectaculos',
    'Transporte & Coches': 'Transporte_Coches',
    'Moda & Belleza': 'Moda_Belleza',
    'Wedding Planners': 'Wedding_Planners',
    'Infraestructura & Sonido': 'Infraestructura_Sonido',
    'Servicios & Regalos': 'Servicios_Regalos'
}

def clean_vendor_name(raw_name):
    if not raw_name:
        return "Proveedor Sin Nombre"
    name = re.sub(r'^(?:Profesional|Premium|Top|Destacado)\s*', '', raw_name, flags=re.I)
    name = re.sub(r'\d{5}\s+[A-Za-zÀ-ÿ\s]+(?:\([A-Za-zÀ-ÿ\s]+\))?.*$', '', name)
    name = re.sub(r'\d/\d\s*\(\d+\).*$', '', name)
    name = re.sub(r'Presta servicio en.*$', '', name, flags=re.I)
    name = re.sub(r'Consulta disponibilidad.*$', '', name, flags=re.I)
    name = name.strip(' -.,|')
...
def normalize_category(raw_cat):
    ...
    for k, v in CATEGORY_MAP.items():
        if k in raw_lower:
            return v
    return 'Servicios Integrales'

def parse_html_vendor(filepath):
    try:
    ...
    slug = fname.replace('.html', '').split('--')[0]

    # Fast JSON-LD scan
    lb = None
    pos = 0
    tag = 'application/ld+json'
    while True:
        ...
        if end == -1: break
        script_body = content[start:end].strip()
        try:
            d = json.loads(script_body, strict=False)
    ...
    # Name
    raw_name = ""
    if lb and lb.get('name'):
        raw_name = lb['name']
    else:
        m_h1 = re.search(r'<h1[^>]*>(.*?)</h1>', content, re.DOTALL | re.I)
        if m_h1:
            raw_name = re.sub(r'<[^>]+>', '', m_h1.group(1)).strip()
        else:
    ...
    # Address & Location
    addr = lb.get('address', {}) if lb else {}
    city = addr.get('addressLocality') or ''
    prov = addr.get('addressRegion') or ''
    street = addr.get('streetAddress') or ''
    postal = addr.get('postalCode') or ''

    if not prov:
        m_reg = re.search(r'\"region\":\s*\"([^\"]+)\"', content)
        if m_reg: prov = m_reg.group(1)
    if not city:
        m_cit = re.search(r'\"city\":\s*\"([^\"]+)\"', content)
        if m_cit: city = m_cit.group(1)

    # Category & URL
    profile_url = lb.get('url') if lb else ''
    if not profile_url:
    ...
    if len(parts) >= 4 and parts[3]:
        category = normalize_category(parts[3])

    # Rating & Reviews
    rating = 5.0
    reviews = 0
    if lb and lb.get('aggregateRating'):
        try:
            rating = float(lb['aggregateRating'].get('ratingValue', 5.0))
            reviews = int(lb['aggregateRating'].get('reviewCount', 0))
        except Exception: pass

    # Price
    price = "Consultar"
    if lb and lb.get('priceRange'):
        p_text = lb['priceRange']
        m_p = re.search(r'(?:desde|partir de)?\s*(\d+[0-9.,]*\s*€)', p_text, re.I)
        if m_p:
            price = m_p.group(1).strip()
        else:
            price = p_text[:35].replace('Precio a partir de', 'Desde').strip()

    # Phone: ONLY extract if authentic in file (never fallback to Edwin's phone)
    phone = (lb.get('telephone') if lb else '') or ''
    if not phone and 'href="tel:' in content:
        t_start = content.find('href="tel:') + 10
        t_end = content.find('"', t_start)
        if t_end != -1:
            raw_t = content[t_start:t_end].strip()
            if raw_t != '+34693693048' and raw_t != '693693048':
                phone = raw_t

    # Image
    img = ""
    if lb and lb.get('logo'):
        img = lb['logo']
    elif lb and lb.get('image'):
        imgs = lb['image']
    ...
    # Google Search URL
    google_search_url = f"https://www.google.com/search?q={clean_name.replace(' ', '+')}+{city.replace(' ', '+')}+{prov.replace(' ', '+')}"

    # Tactical Digital Audit Points
    audit_points = [
    ...
    pitch_hook = f"Hola {clean_name}, te llamo de Productora EAR tras auditar los mejores de {category} en {prov or city}. Tenéis una nota excelente de {rating:.1f}★ ({reviews} opiniones), pero estáis perdiendo eventos porque el cliente actual exige pagar la reserva con tarjeta al instante. En EAR OS os activamos gratis en nuestro catálogo nacional sin cuotas fijas."
    ...
    return {
        ...
        'slug': slug,
        'category': category,
        'province': prov or 'Madrid',
        'city': city or prov or 'Madrid',
        'street': street,
        'postalCode': postal,
...
def main():
    print("=== INICIANDO SINCRONIZADOR PURISTA DE PROVEEDORES HTML ===")
    t0 = time.time()

    # 1. Ensure system vault folder exists
    os.makedirs(SYSTEM_VAULT_DIR, exist_ok=True)
    for sub in SUBFOLDER_MAP.values():
        os.makedirs(os.path.join(SYSTEM_VAULT_DIR, sub), exist_ok=True)

    # 2. Find all HTML files
    html_files = glob.glob(os.path.join(VAULT_INPUT_DIR, "*.html"))
    print(f"Archivos HTML encontrados en bóveda: {len(html_files)}")

    providers = []
    seen = set()

    # Add Edwin Agudelo SSOT #1
    edwin = {
        ...
        'audit_points': [
            "SSOT Sovereign: Sistema de audio Bose F1 812 + S1 Pro homologado a 12 W/pax.",
        ...
        'original_html': 'edwin-agudelo.html'
    }
    ...
    copied_count = 0
    for f in html_files:
        p = parse_html_vendor(f)
        if not p or p['slug'] in seen:
            continue

        seen.add(p['slug'])
        providers.append(p)

        # Copy to organized subfolder
        subfolder = SUBFOLDER_MAP.get(p['category'], 'Servicios_Regalos')
        target_path = os.path.join(SYSTEM_VAULT_DIR, subfolder, os.path.basename(f))
    ...
    print(f"Parseados y validados {len(providers)} proveedores.")
    print(f"Archivos copiados a vault/proveedores_html_indexados: {copied_count}")

    # 4. Save Master JSON in Vault
    vault_json_path = os.path.join(SYSTEM_VAULT_DIR, "CATALOGO_PROVEEDORES_HTML_INDEX.json")
    with open(vault_json_path, "w", encoding="utf-8") as fp:
        json.dump(providers, fp, ensure_ascii=False, indent=2)
    print(f"Catálogo indexado guardado en: {vault_json_path}")

    # 5. Generate Re-Designed CALL CENTER HTML Deck
    generate_call_center_deck_v2(providers)

    dt = time.time() - t0
    print(f"=== SINCRONIZACIÓN PURISTA COMPLETADA EN {dt:.2f}s ===")

def generate_call_center_deck_v2(providers):
...
<head>
  ...
  <title>EAR OS :: Deck de Prospección, Auditoría & Telemarketing de Proveedores</title>
...
<body class="h-full flex flex-col overflow-hidden bg-[#030712] text-slate-200 antialiased selection:bg-red-500 selection:text-white">
  ...
  <header class="h-16 border-b border-slate-800/80 bg-[#060d1d]/90 backdrop-blur-md px-6 flex items-center justify-between z-20 shrink-0">
    <div class="flex items-center gap-4">
      ...
      <div>
        <div class="flex items-center gap-2">
          <span class="font-heading font-bold text-white text-lg tracking-wide">OUTBOUND CALL CENTER & AUDITORÍA</span>
          <span class="px-2 py-0.5 rounded-full text-[10px] font-mono font-bold bg-emerald-500/10 text-emerald-400 border border-emerald-500/20">SSOT PURISTA</span>
        </div>
        <p class="text-xs text-slate-400">Productora EAR :: Sistema de Captación, Análisis Digital & Cierre de Proveedores</p>
      </div>
    </div>
    ...
    <div class="flex items-center gap-3 text-xs font-mono">
      <div class="bg-slate-900/80 border border-slate-800 rounded-lg px-3 py-1.5 flex items-center gap-2">
        <span class="text-slate-400">Catálogo:</span>
  ...
  <div class="flex-1 flex overflow-hidden">
      ...
      <div class="p-4 border-b border-slate-800/80 space-y-3 bg-[#060c1e]/60">
        <div class="relative">
          <input 
            type="text" 
            id="search-box" 
            placeholder="Buscar por nombre limpio, municipio, provincia o especialidad..."
    ...
    <main class="flex-1 flex flex-col bg-[#030712] overflow-y-auto">
      ...
      <div id="deck-empty" class="flex-1 flex flex-col items-center justify-center p-12 text-center text-slate-500">
        ...
        <p class="text-xs text-slate-500 max-w-sm">Haz clic en cualquier proveedor a la izquierda para cargar su auditoría digital, enlace a su web/perfil, guion de llamada y opciones de contacto.</p>
      </div>
        ...
        <div class="p-6 border-b border-slate-800/80 bg-gradient-to-b from-[#0a1329] to-[#040817]">
          ...
          <div class="mt-6 grid grid-cols-3 gap-3">
            ...
            <a id="btn-web" href="#" target="_blank" class="bg-slate-800 hover:bg-slate-700 text-slate-200 font-bold py-2.5 px-3 rounded-xl border border-slate-700 flex items-center justify-center gap-2 text-xs transition">
              <span>🌐</span>
              <span id="btn-web-label">Ver Ficha / Web</span>
            </a>
          </div>
  ...
  <script>
    ...
    function selectProvider(p) {{
      ...
      document.getElementById('deck-rating-badge').textContent = `★ ${{p.rating.toFixed(1)}} (${{p.reviews}} rev)`;
      document.getElementById('deck-price').textContent = p.price;
      document.getElementById('deck-address').textContent = p.street ? `📍 ${{p.street}} (${{p.postalCode}})` : `📍 ${{p.city}}, ${{p.province}}`;
      ...
      const waMsg = `Hola ${{p.name}}, te contacto de Productora EAR (red nacional de eventos). Estábamos revisando tu trabajo en ${{p.category}} (${{p.province}}) y nos gustaría invitarte a sumarte a nuestro catálogo sin cuotas fijas mensuales. Te dejamos tu ficha de acceso para que puedas verificar tus tarifas: ${{p.profile_url}} . ¿Te parece bien que te llamemos para coordinar?`;
    ...
    function exportCRMData() {{
      ...
      rawProviders.forEach(p => {{
        ...
        if (record && (record.status !== 'pending' || record.notes)) {{
          ...
          csv += `"${{p.id}}";"${{p.name}}";"${{p.category}}";"${{p.province}}";"${{p.phone}}";"${{p.profile_url}}";"${{record.status}}";"${{cleanNotes}}"\\n`;
        }}
      }});
...
"""
    ...
    with open(output_html_path, "w", encoding="utf-8") as out:
        out.write(html)
    print(f"Deck HTML actualizado con éxito: {output_html_path} ({len(html)/1024:.1f} KB)")
