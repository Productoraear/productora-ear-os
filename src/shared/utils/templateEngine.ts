...
export type VIMUMETemplate = 'AURA_ONYX' | 'KINETIC_GOLD' | 'INSTITUTIONAL_BLUE' | 'MINIMAL_GLASS';
 ...
 */
export function getTemplateForProvince(province: string): VIMUMETemplate {
  const templates: VIMUMETemplate[] = ['AURA_ONYX', 'KINETIC_GOLD', 'INSTITUTIONAL_BLUE', 'MINIMAL_GLASS'];
 ...
 */
export function getTemplateConfig(template: VIMUMETemplate, location: string) {
  const configs = {
    AURA_ONYX: {
      accentColor: '#ecb613',
