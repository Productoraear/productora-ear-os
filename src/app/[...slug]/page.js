...
import { ShieldCheck, Zap, ArrowRight, Globe, Star, Users, BarChart3 } from 'lucide-react';
 ...
 */
function formatTitle(slugArray) {
 ...
 */
export default async function UniversalOmegaPage({ params }) {
    const { slug } = await params;
    const title = formatTitle(slug);
    ...
    if (isServicio) {
        ...
        const isApexRoute = service.id.includes('premium') ||
            service.id.includes('bespoke') ||
            service.slug.includes('boda') ||
            service.slug.includes('wedding') ||
            slug[1].includes('boda');
        return (<BespokeTemplate title={service.nombre} description={service.descripcion} location={province} serviceId={service.id} keywords={service.keywords} isApex={isApexRoute}/>);
    }
    ...
    if (slug.length >= 2 && (isArsenal || isWeddings || isBusiness)) {
        ...
        return (<BespokeTemplate title={itemTitle} description={`Protocolo de actuación institucional para ${itemTitle}. Autoridad técnica y soporte VIMUME OS certificado.`} location="España" serviceId={slug.join('_')} keywords={[itemTitle, slug[0], 'VIMUME OS', 'Autoridad']} isApex={true}/>);
    }
    ...
    return (<div className="min-h-screen bg-[#050505] text-white selection:bg-[#ecb613]/30 selection:text-white font-inter">
      {/* NAVEGACIÓN BLINDADA */}
      ...
      <main className="pt-32 pb-20 space-y-32">
        ...
        <section className="relative px-6 max-w-7xl mx-auto overflow-hidden">
          ...
          <div className="relative z-10 space-y-10">
            ...
            <p className="text-xl md:text-3xl text-white/50 max-w-4xl leading-tight font-light italic">
              {isCategory ? (`Protocolos de impacto y gestión avanzada para la vertical de ${title}. Optimizando cada nodo de servicio.`) : (`"No solo gestionamos proyectos en ${title}. Garantizamos la excelencia operativa mediante el despliegue de protocolos de alta autoridad."`)}
            </p>

            <div className="flex flex-col sm:flex-row gap-6 pt-10">
              <Link href="/admin/configurador" className="bg-[#ecb613] text-black px-16 py-7 rounded-2xl font-black uppercase text-xs tracking-[0.4em] shadow-[0_20px_50px_rgba(212,168,85,0.2)] hover:scale-105 transition-all flex items-center justify-center gap-4">
                RESERVAR PROYECTO <Zap className="w-5 h-5"/>
              </Link>
              <Link href="#servicios" className="bg-white/5 border border-white/10 text-white px-16 py-7 rounded-2xl font-black uppercase text-xs tracking-[0.4em] hover:bg-white/10 transition-all flex items-center justify-center gap-4 backdrop-blur-xl">
        ...
        ].map((p, i) => (<div key={i} className="space-y-4 border-l border-white/5 pl-8">
        ...
        {!isCategory ? (<section id="servicios" className="px-6 max-w-7xl mx-auto space-y-16">
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
                    {SERVICIOS.map((s, i) => (<Link key={i} href={`/${slug[0]}/${s.slug}`} className="group p-10 rounded-[2.5rem] border border-white/5 bg-zinc-950/50 hover:border-[#ecb613]/30 transition-all duration-500 relative overflow-hidden">
            ...
            </section>) : slug[0] === 'artistas' && (<section id="servicios" className="px-6 max-w-7xl mx-auto space-y-24">
                {/* 👑 MASTER ARTIST SPOTLIGHT (PACIENTE CERO) */}
                {FEATURED_ARTISTS.filter(a => a.isStrategicFocus).map((ceo, i) => (<div key={i} className="relative group">
                                ...
                                <div className="grid grid-cols-2 gap-6">
                                    <div className="space-y-2">
                                        ...
                                        <div className="flex flex-wrap gap-2">
                                            {ceo.roles?.map((r, ri) => (<span key={ri} className="text-[10px] font-bold text-white/40">/ {r}</span>))}
                                        </div>
                                    </div>
                                    <div className="space-y-2">
                ...
                <div className="grid grid-cols-1 md:grid-cols-4 gap-8">
                    {FEATURED_ARTISTS.filter(a => !a.isStrategicFocus).map((artist, i) => (<div key={i} className="group space-y-6">
        ...
        <section className="bg-[#ecb613] py-32 px-6 text-black text-center space-y-12">
            ...
            <Link href="/admin/configurador" className="inline-block bg-black text-[#ecb613] px-20 py-8 rounded-2xl font-black uppercase text-sm tracking-[0.5em] hover:scale-105 transition-all shadow-2xl">
                ACTIVAR PROYECTO
            </Link>
        </section>
      </main>
