...
export async function GET(request) {
    try {
        ...
        if (isThreat) {
            return NextResponse.json({ error: 'Blocked by S-Class Security Shield: Automation attempt logged.' }, { status: 403 });
        }
        ...
        return NextResponse.json({ error: 'Internal Server Error during profile search' }, { status: 500 });
    }
}
