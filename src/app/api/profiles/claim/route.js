...
export async function POST(request) {
    try {
        ...
        if (!profileId || !userId) {
            return NextResponse.json({ error: 'Missing profileId or userId in request body' }, { status: 400 });
        }
        ...
        if (!user) {
            return NextResponse.json({ error: `User with ID "${userId}" not found in database` }, { status: 404 });
        }
        ...
        if (!profile) {
            return NextResponse.json({ error: `Provider profile with ID "${profileId}" not found` }, { status: 404 });
        }
        // 3. Comprobar si ya está reclamado
        if (profile.userId) {
            if (profile.userId === userId) {
                return NextResponse.json({ message: 'You have already claimed this profile.', profile }, { status: 200 });
            }
            return NextResponse.json({ error: 'This profile has already been claimed by another user.' }, { status: 409 });
        }
