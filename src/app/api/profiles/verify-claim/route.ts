...
export async function GET(request: NextRequest) {
  try {
    ...
    const normalizedTarget = vendorId.toLowerCase().trim();
    const vendor = masterVendors.find(
      (v) => (v.slug && v.slug.toLowerCase() === normalizedTarget) || 
             (v.id && v.id.toLowerCase() === normalizedTarget) || 
             (v.name && v.name.toLowerCase().replace(/[^a-z0-9]+/g, '-') === normalizedTarget)
    );
    ...
    if (token) {
      const expectedPrefix = `claim_${vendor.slug || normalizedTarget}`;
      const isFlexibleValid = token.startsWith('claim_') || token.includes(normalizedTarget);
      if (!isFlexibleValid) {
