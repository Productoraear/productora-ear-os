...
export default async function DynamicMariachiPage({ params }: Props) {
  const provincia = PROVINCIAS.includes(params.provincia) ? params.provincia : 'Otras provincias';
  const evento = MATRIX_EVENTOS.includes(params.evento) ? params.evento : 'otros eventos';

  if (!PROVINCIAS.includes(params.provincia) || !MATRIX_EVENTOS.includes(params.evento)) {
    return notFound();
  }
