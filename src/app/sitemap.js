...
export default function sitemap() {
    ...
    const occasionPages = OCASIONES.map(o => ({ url: `${baseUrl}/ocasiones/${o.slug}`, lastModified: now, priority: 0.8, changeFrequency: 'weekly' }));
    const guidePages = GUIAS.map(g => ({ url: `${baseUrl}/guias/${g.slug}`, lastModified: now, priority: 0.7, changeFrequency: 'monthly' }));
    const servicePages = SERVICIOS.map(s => ({ url: `${baseUrl}/servicios/${s.slug}`, lastModified: now, priority: 0.8, changeFrequency: 'weekly' }));
    const matrixPages = HIGH_VALUE_VARIANTS.map(v => ({ url: `${baseUrl}/artistas/${v.slug}`, lastModified: now, priority: 0.9, changeFrequency: 'weekly', indexable: v.indexable ?? true }));
    const dynamicPages = [];
    SERVICIOS.forEach(service => {
        PROVINCIAS.forEach(province => {
            ...
            dynamicPages.push({ url, lastModified: now, changeFrequency: 'monthly', priority, indexable: priority >= 0.8 });
        });
    });
    const provinceEntries = PROVINCIAS.map(p => ({ url: `${baseUrl}/servicios/mariachis/${p}`, lastModified: now, priority: 0.8, changeFrequency: 'monthly' }));
