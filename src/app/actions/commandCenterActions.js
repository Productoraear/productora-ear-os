 ...
 */
export async function getWaybills(email) {
 ...
 */
export async function getAuraWalletAndLedgers(email) {
    const auth = await authorizeUser(email);
    if (!auth.authorized) {
        throw new Error("UNAUTHORIZED_ACCESS: Clearance level S-Class required.");
    }
 ...
 */
export async function getSystemFinancials(email) {
    const auth = await authorizeUser(email);
 ...
 */
export async function runAstraPrediction(email, input) {
    const auth = await authorizeUser(email);
