  ...
  return (
    <section className="relative min-h-[90vh] flex items-center justify-center overflow-hidden bg-black pt-20">
      ...
      <div className="absolute inset-0 z-0">
        <div className="absolute inset-0 bg-[radial-gradient(circle_at_50%_50%,rgba(212,168,85,0.1),transparent_70%)]" />
        <motion.div 
          animate={{ 
            scale: [1, 1.1, 1],
            opacity: [0.3, 0.5, 0.3] 
          ...
          className="absolute inset-0 bg-[url('/images/wedding.png')] bg-cover bg-center grayscale opacity-30"
        />
      ...
      <div className="container-custom relative z-10 text-center">
        ...
        >
          <Zap size={14} className="text-[#d4a855] animate-pulse" />
          <span className="text-[10px] font-black uppercase tracking-[0.4em] text-white/60">
            S-Class Marketplace OS <span className="text-[#d4a855]">V164.1</span>
          </span>
        </motion.div>
        ...
        >
          Diseña lo <br />
          <span className="text-transparent bg-clip-text bg-gradient-to-r from-white via-[#d4a855] to-white/20">
            Inevitable
          </span>
        </motion.h1>
        ...
        <motion.form 
          ...
          className={`relative max-w-5xl mx-auto p-4 bg-white/5 backdrop-blur-3xl rounded-[3rem] border transition-all duration-700 ${
            isFocused ? 'border-[#d4a855]/40 shadow-[0_0_50px_rgba(212,168,85,0.15)] scale-105' : 'border-white/10'
          }`}
        >
          <div className="flex flex-col md:flex-row items-center gap-2">
            ...
            <button 
              type="submit"
              className="w-full md:w-auto h-20 px-12 bg-[#d4a855] text-black rounded-full flex items-center justify-center gap-4 hover:scale-105 active:scale-95 transition-all shadow-xl shadow-[#d4a855]/20 group/btn"
            >
        ...
        >
          ...
          {['Gala Institucional', 'Incentivos B2B', 'Luxury Destination'].map((trend) => (
            <button key={trend} className="text-[10px] font-bold uppercase tracking-widest text-white/40 hover:text-[#d4a855] transition-colors">
              {trend}
            </button>
          ))}
        </motion.div>
