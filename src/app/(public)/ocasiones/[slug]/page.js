...
export default function OccasionHubPage({ params }) {
    const occasion = getOccasionBySlug(params.slug);
    if (!occasion)
        notFound();
    return (<main className="min-h-screen bg-[#050505] text-white pt-32 pb-20 overflow-hidden relative">
      {/* BACKGROUND ACCENT */}
      ...
      <div className="container-custom relative z-10">
        ...
        <header className="max-w-4xl mb-24">
          <span className="text-[10px] font-black uppercase tracking-[0.5em] mb-4 block" style={{ color: occasion.accentColor }}>
            {occasion.intent}
          </span>
        ...
        <section className="mb-32">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-10">
            ...
            <MarketplaceCard index={0} id="mariachi-gala" title="Mariachi Gala S-Class" location="Nacional" category="MÚSICA ÉLITE" price="2.800" rating={5.0} image="https://images.unsplash.com/photo-1514525253361-bee8718a300a?auto=format&fit=crop&q=80&w=1000"/>
            {/* ... más cards ... */}
          </div>
        </section>
        ...
        <footer className="pt-20 border-t border-white/5 opacity-30 hover:opacity-100 transition-opacity">
          <div className="grid grid-cols-2 md:grid-cols-4 gap-8">
            {getAllOccasions().map(occ => (<a key={occ.slug} href={occ.canonicalPath} className="text-[10px] font-bold uppercase tracking-widest hover:text-[#d4a855] transition-colors">
                {occ.title}
              </a>))}
          </div>
        </footer>
