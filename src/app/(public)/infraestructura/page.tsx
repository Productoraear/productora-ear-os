...
export default function InfraestructuraSilo() {
  ...
  // Simulación de HunterCollector (Arbitraje de Caché)
  useEffect(() => {
    const interval = setInterval(() => {
      const newDemand = Math.floor(Math.random() * (99 - 80 + 1) + 80);
      setDemandLevel(newDemand);
      // El margen sube si la demanda es mayor al 90%
  ...
  return (
    <div className="min-h-screen bg-[#050505] text-white selection:bg-blue-500/30">
        ...
        <div className="max-w-7xl mx-auto relative z-10 text-center">
          ...
          >
            ...
            <p className="text-xl md:text-3xl font-light text-white/50 max-w-4xl mx-auto italic mb-12">
              "Proveedor oficial de potencia sónica, iluminación táctica y logística redundante para Eventos Masivos, Ayuntamientos y el Mundial 2026."
            </p>

            {/* SHADOW BOOKING & ARBITRAJE HUD */}
            <div className="inline-flex flex-col md:flex-row items-center gap-6 p-6 rounded-2xl bg-blue-900/10 border border-blue-500/20 backdrop-blur-xl">
              <div className="flex items-center gap-3">
                ...
                <span className="text-[10px] font-black uppercase tracking-widest text-white/60">Saturación de Red</span>
              </div>
              ...
              <div className="flex items-center gap-2">
                ...
                <span className="text-[10px] font-black uppercase tracking-widest text-blue-500">HunterCollector Activo</span>
              </div>
            </div>
          </motion.div>
        </div>
            ...
            {[
              {
                icon: <Radio size={24} />,
                title: "FBT 118 SA",
                subtitle: "Subwoofers S-Class",
                desc: "Presión sonora de grado institucional. Cobertura acústica militar para grandes aforos y plazas abiertas."
              },
              {
                icon: <Cpu size={24} />,
                title: "Shure QLXD",
                subtitle: "Microfonía Digital",
                desc: "Transmisión encriptada AES-256. Cero interferencias en entornos saturados como recintos deportivos."
              },
              {
                icon: <HardDrive size={24} />,
                title: "Redundancia 100%",
                subtitle: "Arquitectura Fail-Safe",
                desc: "Mesas de respaldo, cableado doble y energía ininterrumpida. Si un nodo cae, el sistema no parpadea."
              },
              {
                icon: <Zap size={24} />,
                title: "Iluminación Táctica",
                subtitle: "Beam & Wash",
                desc: "Atmósferas volumétricas controladas vía Art-Net. Escenografía dinámica adaptada al protocolo B2G."
              }
            ].map((tech, i) => (
              <motion.div 
                key={i}
      ...
      {/* HUNTERCOLLECTOR & PRICING ENGINE */}
          ...
          <div className="glass-panel p-12 rounded-[3rem] border border-blue-500/20 bg-blue-900/5 relative overflow-hidden">
            {demandLevel > 90 && (
              <div className="absolute top-0 left-0 w-full p-2 bg-red-500 text-center flex items-center justify-center gap-2">
                ...
                <span className="text-[9px] font-black uppercase tracking-widest text-black">ALERTA DE INVENTARIO: COMPETENCIA LOCAL SATURADA</span>
              </div>
            )}
            ...
            <div className={`mt-8 grid grid-cols-1 md:grid-cols-2 gap-16 items-center ${demandLevel > 90 ? 'pt-4' : ''}`}>
              <div>
                <h2 className="text-4xl md:text-5xl font-black uppercase tracking-tighter leading-none mb-6">
                  Despliegue <br />
                  <span className="text-blue-500">Institucional</span>
                </h2>
                <p className="text-white/40 font-bold uppercase text-xs leading-relaxed mb-8">
                  El motor OmegaCrawler ha detectado una alta saturación en los proveedores locales convencionales. Nuestra infraestructura S-Class está lista para su licitación.
                </p>
                <Link href="/cotizador" className="inline-flex items-center justify-center px-10 py-5 bg-blue-500 text-black font-black uppercase tracking-[0.2em] text-[10px] rounded-full hover:bg-white transition-all shadow-[0_0_30px_rgba(59,130,246,0.3)]">
                  INICIAR DESPLIEGUE TÉCNICO
                </Link>
              </div>

              <div className="p-8 rounded-3xl bg-black/50 border border-white/5 space-y-6">
                <div className="flex justify-between items-center">
                  <span className="text-[10px] font-black uppercase tracking-widest text-white/40">Tarifa Base (Día)</span>
                  <span className="text-lg font-black">2.500€</span>
                </div>
                <div className="flex justify-between items-center">
                  <span className="text-[10px] font-black uppercase tracking-widest text-blue-500">Arbitraje Dinámico (+{dynamicMargin}%)</span>
                  <span className="text-lg font-black text-blue-500">+{2500 * (dynamicMargin / 100)}€</span>
                </div>
                <div className="h-px bg-white/10 w-full" />
                <div className="flex justify-between items-center">
                  <span className="text-xs font-black uppercase tracking-widest">Presupuesto Mínimo</span>
                  <span className="text-3xl font-black text-white">{2500 + (2500 * (dynamicMargin / 100))}€</span>
                </div>
                <p className="text-[8px] text-white/30 uppercase tracking-widest text-center">Tarifa ajustada algorítmicamente basada en ocupación zonal.</p>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="max-w-7xl mx-auto px-8">
          <div className="flex flex-col items-center justify-center p-8 bg-blue-500/5 border border-blue-500/20 rounded-2xl mb-16 text-center">
            <div className="flex items-center gap-3 mb-4">
              <Shield className="text-blue-500" size={24} />
              <span className="text-[10px] font-black uppercase tracking-[0.3em] text-blue-500">Productora EAR GOLD B2G</span>
            </div>
            <p className="text-[10px] text-white/60 uppercase font-black tracking-widest max-w-2xl">
              Equipamiento técnico homologado para licitaciones públicas y eventos masivos de nivel internacional.
            </p>
          </div>
        </div>
      </footer>
