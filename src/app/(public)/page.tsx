...
export default function Home() {
  ...
  return (
    <main className="min-h-screen bg-[#050505] text-white flex flex-col font-sans selection:bg-[#ecb613] selection:text-black relative overflow-x-hidden">
      {/* 1. PORTADA SOBERANA: 4 GATEWAYS DE ACCESO DIRECTO S-CLASS */}
      <CinematicHeroSClass />

      {/* 2. BARRA DE ACCESOS DIRECTOS DE ALTA VELOCIDAD */}
      <div className="border-t border-white/5 bg-[#0a0a0f]/90 py-4 px-6 overflow-x-auto no-scrollbar z-20">
          ...
          <div className="flex items-center gap-2.5">
            {occasionLinks.map((occ, i) => (
              <Link
                key={i}
                href={occ.href}
                className="px-4 py-2 rounded-full bg-white/5 hover:bg-[#ecb613] hover:text-black border border-white/10 text-xs font-mono text-white/80 transition-all flex items-center gap-2 group shadow-sm hover:scale-105"
              >
                <span>{occ.title}</span>
                <span className="text-[9px] opacity-60 font-bold group-hover:opacity-100 uppercase">{occ.tag}</span>
              </Link>
            ))}
          </div>
        </div>
