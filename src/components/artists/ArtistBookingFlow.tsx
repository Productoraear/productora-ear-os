...
export const ArtistBookingFlow: React.FC<ArtistBookingFlowProps> = ({ city, eventType }) => {
  ...
  const formattedDate = eventDate 
    ? new Date(eventDate + 'T00:00:00').toLocaleDateString('es-ES', { day: 'numeric', month: 'long', year: 'numeric' }) 
    : '';
  ...
  return (
    <div className="bg-[#0b0b0b] border border-white/5 rounded-[2.5rem] sm:rounded-[3.5rem] p-6 sm:p-12 space-y-8 relative overflow-hidden">
      ...
      <div className="flex bg-white/5 border border-white/10 rounded-2xl p-1 relative z-10">
        <button
          ...
          className={`flex-1 py-3 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all flex items-center justify-center gap-2 min-h-[44px] ${
            flowType === 'B2C' 
      ...
      <div>
        ...
        <h3 className="text-2xl sm:text-3xl font-black uppercase italic tracking-tighter text-white font-syne">
        ...
        <p className="text-white/40 text-xs font-bold leading-relaxed mt-1">
          {flowType === 'B2C' 
            ? 'Bloquea tu fecha preferente con confirmación de agenda y atención prioritaria del equipo de Edwin Agudelo.'
      ...
      {success ? (
        <div className="bg-emerald-500/10 border border-emerald-500/20 rounded-3xl p-6 sm:p-8 text-center space-y-4">
          ...
          <h4 className="text-xl font-black uppercase text-white tracking-tight">
            {flowType === 'B2C' ? '¡Fecha Registrada con Éxito!' : '¡Solicitud Registrada en Despacho!'}
          </h4>
          <p className="text-white/60 text-xs sm:text-sm max-w-md mx-auto leading-relaxed">
            {flowType === 'B2C' 
              ? `Se ha registrado tu pre-reserva de fecha para el ${formattedDate} ${eventType && city ? `para tu espectáculo de ${eventType} en ${city}` : ''}. Nuestro manager se pondrá en contacto en menos de 2 horas para formalizar la reserva.`
              : `La solicitud de presupuesto para ${institutionName || 'su entidad pública'} para el ${formattedDate} ha sido registrada con el código B2G-CONFIRMED. Generando propuesta digital de Contrato Menor...`}
          </p>
          <div className="pt-4 flex flex-col sm:flex-row justify-center gap-3">
            <a
              href={`/cotizador?items=cuarteto-gala&date=${eventDate}`}
              className="py-3 px-6 rounded-xl bg-[#ecb613] text-black font-black text-xs uppercase tracking-wider min-h-[44px] flex items-center justify-center gap-2 shadow-lg shadow-[#ecb613]/20 active:scale-95 transition-all"
            >
              <span>Ver Desglose en Cotizador</span>
            </a>
            {flowType === 'B2G' && (
              <span className="inline-flex items-center justify-center gap-1.5 text-xs font-black uppercase tracking-widest text-[#ecb613] bg-[#ecb613]/10 px-4 py-3 rounded-xl border border-[#ecb613]/25 cursor-pointer hover:bg-white hover:text-black transition-all min-h-[44px]">
                <FileText size={14} /> Descargar Borrador en PDF
              </span>
            )}
          </div>
        </div>
          ...
          {flowType === 'B2C' ? (
            ...
            <div className="space-y-6">
              ...
              <button
                type="submit"
                disabled={loading}
                className="w-full bg-[#ecb613] text-black font-black uppercase tracking-[0.2em] text-xs py-5 rounded-2xl flex items-center justify-center gap-3 hover:bg-white transition-all shadow-xl shadow-[#ecb613]/10 disabled:opacity-60 min-h-[48px] active:scale-95"
              >
                <CalendarIcon size={16} /> {loading ? 'Registrando Pre-reserva...' : 'Bloquear Fecha de Actuación'}
              </button>
            </div>
          ) : (
