...
function calculateAuraLevel(vendor: BodasVendor): number {
...
function calculateRoiProjected(vendor: BodasVendor): number {
...
function calculateTechnicalReliability(vendor: BodasVendor): number {
  const rating = vendor.rating || 0;
...
async function main() {
  ...
  let vendors: BodasVendor[] = JSON.parse(data);
  ...
  for (let i = 0; i < vendors.length; i += batchSize) {
    ...
    await Promise.all(
      batch.map(async (vendor) => {
        try {
          if (!vendor.name) return; // Skip si no hay nombre
