...
export const DemandEngine = () => {
  ...
  const metrics: FunnelMetric[] = [
    { label: 'CAC Real', value: '€42.30', change: '-12%', trend: 'up' },
    { label: 'Tasa de Conversión', value: '8.4%', change: '+2.1%', trend: 'up' },
    { label: 'Indexación IA (AEO)', value: '98%', change: 'Élite', trend: 'up' },
    { label: 'Soberanos x Sem', value: '24', change: '+5', trend: 'up' },
  ];
  ...
  return (
    ...
    >
      {/* DECORACIÓN DE FONDO */}
      <div className="absolute top-0 right-0 w-64 h-64 bg-[#d4a855]/5 blur-[100px] rounded-full -translate-y-1/2 translate-x-1/2" />
      
      <div className="flex flex-col lg:flex-row gap-8">
        {/* SIDEBAR: CONTROL DE MISIÓN */}
        <div className="w-full lg:w-1/3 space-y-6">
          <div className="flex items-center gap-3 mb-2">
            <div className="p-2 bg-[#d4a855] rounded-lg shadow-lg shadow-[#d4a855]/20">
              <Rocket className="w-5 h-5 text-black" />
            </div>
            <div>
              <h2 className="text-xl font-black uppercase tracking-tighter text-white">MOTOR DE DEMANDA</h2>
              <p className="text-[10px] text-white/40 tracking-widest uppercase font-bold">Nivel 7: Marketing Autónomo</p>
            </div>
          </div>

          <p className="text-xs text-white/60 leading-relaxed uppercase tracking-wider font-medium">
            El sistema genera, optimiza y despliega embudos de conversión de alta fidelidad 
            utilizando datos de la red de vampirización y AEO para dominancia absoluta en IAs.
          </p>
          ...
          <button 
            ...
            className={`w-full py-5 rounded-xl font-black text-[10px] tracking-[0.3em] uppercase transition-all flex items-center justify-center gap-3 ${
              ...
              : 'bg-[#d4a855] text-black hover:scale-[1.02] active:scale-[0.98] shadow-[0_0_30px_rgba(212,168,85,0.3)] border-none'
            }`}
          >
            {isGenerating ? <RefreshCw className="w-4 h-4 animate-spin" /> : <Zap className="w-4 h-4" />}
            {isGenerating ? `GENERANDO EMBUDO: ${progress}%` : 'INICIAR CAMPAÑA AUTÓNOMA'}
          </button>
        </div>
        ...
        <div className="flex-1 space-y-6">
          <div className="flex justify-between items-center">
            <h3 className="text-xs font-black text-white/80 uppercase tracking-widest flex items-center gap-2">
              <Layout className="w-4 h-4 text-[#d4a855]" />
              ...
              {landings.map((lp, idx) => (
                ...
                >
                  <div className="flex items-center gap-5">
                    <div className="w-12 h-12 rounded-xl bg-white/5 flex items-center justify-center border border-white/10 group-hover:border-[#d4a855]/30 group-hover:bg-[#d4a855]/5 transition-all">
                      <Target className="w-6 h-6 text-white/30 group-hover:text-[#d4a855]" />
                    </div>
                    <div>
                      <h4 className="text-xs font-black uppercase text-white tracking-wider group-hover:text-[#d4a855] transition-colors">{lp.title}</h4>
                      <div className="flex items-center gap-3 mt-1">
                        <span className="text-[10px] text-white/20 font-mono tracking-tighter">{lp.url}</span>
                        <span className="text-[8px] px-2 py-0.5 bg-white/5 text-white/40 rounded border border-white/10 font-black uppercase group-hover:border-[#d4a855]/30 group-hover:text-[#d4a855]">
                          {lp.persona}
                        </span>
                      </div>
                    </div>
                  ...
                  <div className="flex items-center gap-10">
                    ...
                    <div className="w-10 h-10 rounded-full border border-white/10 flex items-center justify-center group-hover:border-[#d4a855]/50 transition-all bg-white/[0.02] group-hover:bg-[#d4a855]/10">
                      <ExternalLink className="w-4 h-4 text-white/20 group-hover:text-[#d4a855]" />
                    </div>
                  </div>
                </motion.div>
              ))}
          ...
          <div className="mt-8 glass-pane bg-white/[0.01] p-8 border-dashed border-white/10 rounded-[2rem]">
            <div className="flex items-center gap-3 mb-6">
              ...
              <h3 className="text-[10px] font-black text-white/60 uppercase tracking-[0.3em]">Radar de Visibilidad AEO (Gemini / Claude / SearchGPT)</h3>
            </div>
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-10">
              ...
              <div className="space-y-3">
                <div className="flex justify-between text-[10px] uppercase text-white/30 font-black">
                  ...
                  <span className="text-[#d4a855]">Élite</span>
                </div>
                <div className="h-1 bg-white/5 rounded-full overflow-hidden">
                  <motion.div 
                    initial={{ width: 0 }}
