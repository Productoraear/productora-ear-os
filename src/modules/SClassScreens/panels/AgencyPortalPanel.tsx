...
export default AgencyPortalPanel;
export function AgencyPortalPanel() {
  ...
  return (
    <div className="bg-[#0a0a0a] min-h-screen text-white font-sans selection:bg-[#d4af37] selection:text-black p-4 md:p-12 overflow-hidden relative rounded-[3rem] border border-white/5">
              ...
              {[
                ...
                { id: 'assets', label: 'Activos' }
                 ...
                 <button 
                  ...
                  className={`px-8 h-14 rounded-full font-black uppercase tracking-widest text-[10px] transition-all border ${activeTab === tab.id ? 'bg-white text-black border-white' : 'bg-transparent text-zinc-500 border-white/10 hover:border-white/20'}`}
                 >
                    {tab.label}
                 </button>
              ))}
        ...
        <div className="grid grid-cols-1 xl:grid-cols-12 gap-12">
           ...
           <div className="xl:col-span-8 space-y-12">
              <section className="bg-zinc-900/20 rounded-[3rem] border border-white/5 p-12 overflow-hidden relative group">
                 ...
                 <h2 className="text-2xl font-black uppercase tracking-tighter mb-8 italic">Despliegue de Activos <span className="text-[#d4af37]/40">Tiempo Real</span></h2>
                       ...
                       <div key={asset} className="bg-black/40 p-10 rounded-[2.5rem] border border-white/5 hover:border-white/10 transition-all space-y-6">
                          <div className="flex justify-between items-start">
                             ...
                             <div className="text-right">
                                <div className="text-xl font-bold font-mono tracking-tighter">CAMIÓN-0{asset}</div>
                      ...
                      { time: '21:00', task: 'Soundcheck Nivel III (Aislamiento)', status: 'future' },
                       ...
                       <div key={i} className={`p-8 rounded-[2rem] border flex items-center gap-12 ${step.status === 'current' ? 'bg-[#d4af37]/5 border-[#d4af37]/30' : 'bg-transparent border-white/5 opacity-40'}`}>
                          ...
                          <div className="flex-1">
                             ...
                             <p className="text-[10px] uppercase font-black tracking-widest text-zinc-500 mt-1">Estado: {step.status === 'done' ? 'COMPLETADO' : step.status === 'current' ? 'EN CURSO' : 'PENDIENTE'}</p>
                          </div>
