 ...
 */
function runAnalyticalFallback(input) {
    ...
    if ((lowercaseDest.includes("ibiza") || lowercaseDest.includes("palma") || lowercaseDest.includes("baleares") || lowercaseDest.includes("canarias")) &&
        !(lowercaseOrigin.includes("ibiza") || lowercaseOrigin.includes("palma"))) {
        geoPriceMultiplier = 2.2; // High island tariff multiplier
    }
    else if ((lowercaseDest.includes("madrid") && lowercaseOrigin.includes("barcelona")) ||
...
export class AstraPredictiveEngine {
    /**
     * Evaluates pricing & risk logs using dynamic RAG prompts + Gemini.
     */
    async predict(input) {
        if (!GEMINI_API_KEY) {
        ...
        try {
            ...
            // Compile contextual data for RAG processing
