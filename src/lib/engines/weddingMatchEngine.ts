...
export type AuraType = 'elite_luxury' | 'boutique_intimate' | 'modern_hightech' | 'fiesta_salvaje';
export type ClimaxType = 'elegancia_eterna' | 'efecto_festival' | 'disrupcion_cool';
export type ServiceCategory = 'MUSICA_LIVE' | 'DJ' | 'MARIACHI' | 'FOTOGRAFIA' | 'VIDEO' | 'CATERING' | 'DECORACION' | 'ILUMINACION' | 'WEDDING_PLANNER';
...
export interface WeddingPreferences {
    ...
    date: string;                    
    guestCount: number;              
    priorities: ServiceCategory[];   
    venueType?: 'indoor' | 'outdoor' | 'mixed';
    customWeights?: Map<ServiceCategory, number>;
...
const AURA_CLIMAX_COMPAT: Record<AuraType, ClimaxType[]> = {
...
const BUDGET_DISTRIBUTION: Record<ServiceCategory, number> = {
    MUSICA_LIVE: 0.15,
    DJ: 0.08,
    MARIACHI: 0.12,
    FOTOGRAFIA: 0.18,
...
function calculateAuraScore(provider: ProviderProfile, prefs: WeddingPreferences): number {
    let score = 0;
    if (provider.auraFit.includes(prefs.aura)) score += 70;
    ...
    if (climaxMatch) score += 20;
    if (provider.climaxFit.includes(prefs.climax)) score += 10;
    return Math.min(100, score);
}

function calculateBudgetScore(provider: ProviderProfile, prefs: WeddingPreferences): number {
    ...
    if (avgPrice <= categoryBudget * 0.5) return 60 + (avgPrice / (categoryBudget * 0.5)) * 20;
    if (avgPrice <= categoryBudget) return 80 + (1 - (avgPrice / categoryBudget)) * 20;
...
function calculateProximityScore(provider: ProviderProfile, prefs: WeddingPreferences): number {
    const city = prefs.city.toLowerCase().trim();
    if (provider.territory.some(t => t.toLowerCase().trim() === city)) return 100;
...
function calculateRatingScore(provider: ProviderProfile): number {
...
function calculateAvailabilityScore(provider: ProviderProfile, prefs: WeddingPreferences): number {
    if (!provider.availability || provider.availability.length === 0) return 50;
    return provider.availability.includes(prefs.date) ? 100 : 0;
}

function calculateResponseSpeedScore(provider: ProviderProfile): number {
    if (provider.responseTimeHours <= 1) return 100;
    if (provider.responseTimeHours <= 4) return 85;
    if (provider.responseTimeHours <= 12) return 70;
    if (provider.responseTimeHours <= 24) return 50;
    if (provider.responseTimeHours <= 48) return 25;
    return 10;
}

function generateMatchReason(provider: ProviderProfile, breakdown: ScoreBreakdown, prefs: WeddingPreferences): string {
    const reasons: string[] = [];
    if (breakdown.auraScore >= 80) reasons.push(`Encaja perfectamente con tu Aura "${prefs.aura.replace(/_/g, ' ')}"`);
    if (breakdown.ratingScore >= 80) reasons.push(`${provider.rating}★ en ${provider.totalEvents} eventos`);
    if (breakdown.proximityScore >= 70) reasons.push(`Disponible en tu zona`);
    if (breakdown.budgetScore >= 80) reasons.push(`Dentro de tu presupuesto`);
    if (breakdown.responseSpeedScore >= 85) reasons.push(`Respuesta en menos de 4h`);
    return reasons.join(' · ') || 'Match recomendado por ASTRA';
}

function estimatePrice(provider: ProviderProfile, prefs: WeddingPreferences): number {
...
export function runMatchEngine(
...
): Map<ServiceCategory, MatchResult[]> {
    const resultsByCategory = new Map<ServiceCategory, MatchResult[]>();
    const providersByCategory = new Map<ServiceCategory, ProviderProfile[]>();
    ...
    for (const [category, categoryProviders] of providersByCategory) {
        const scored: MatchResult[] = categoryProviders.map(provider => {
...
export function suggestBudgetDistribution(
    totalBudget: number,
    priorities: ServiceCategory[]
): Map<ServiceCategory, { suggested: number; min: number; max: number }> {
    const distribution = new Map<ServiceCategory, { suggested: number; min: number; max: number }>();
    ...
    for (const [cat, weight] of Object.entries(adjustedWeights)) {
        ...
        distribution.set(cat as ServiceCategory, {
            suggested,
...
export const CATEGORY_UI: Record<ServiceCategory, { label: string; emoji: string; description: string }> = {
