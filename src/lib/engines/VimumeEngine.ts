...
export interface VimumeSession {
  id: string;
  level: number;
  title: string;
  focus: 'memoria' | 'emocion' | 'motricidad' | 'social';
  frequency: string;
  description: string;
}

export const VIMUME_PROTOCOLS: VimumeSession[] = [
  {
    id: 'V-001',
    level: 1,
...
export class VimumeEngine {
   ...
   */
  static calculateICP(patientAge: number, sessionsCompleted: number, engagementScore: number): number {
   ...
   */
  static generateClinicalInsight(patientData: any): string {
    return `
