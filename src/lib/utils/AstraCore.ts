...
import { S_CLASS_THEME, OPERATIONAL_PROTOCOLS, ASTRA_IDENTITIES } from '../constants/SClassNexus';
import { VimumeEngine } from '../engines/VimumeEngine';
 ...
 */
export const washData = (text: string): string => {
  if (!text) return '';
  let cleaned = text;
 ...
 */
export const validateLeadGeo = (location: string): boolean => {
 ...
 */
export const calculateAlphaScore = (orders: any[]): number => {
 ...
 */
export const generateAstraVerdict = (latestOrder: any, latestFleet: any, ordersList: any[]): string => {
 ...
 */
export const generateAstraSystemPrompt = (context: 'ventas' | 'vimume' | 'operativo' = 'operativo'): string => {
  const base = `
